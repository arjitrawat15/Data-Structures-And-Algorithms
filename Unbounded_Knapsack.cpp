...
//but here we have a infinite supply of items therefore we can choose a item any no of times to put it in the knapsack.
#include<iostream>
#include<vector>
#include<climits>
using namespace std;
int recursion(int ind,int W,vector<int>& wt,vector<int>& val) // TC-> expo
{
    ...
    if(wt[ind]<=W)
    {
        take=val[ind]+recursion(ind,W-wt[ind],wt,val);
    }
...
int memo(int ind,int W,vector<int>& wt,vector<int>& val,vector<vector<int>>& dp) // TC-> O(N X W) SC-> O(N X W) + O(N)
{
    ...
    if(wt[ind]<=W)
    {
        take=val[ind]+memo(ind,W-wt[ind],wt,val,dp);
    }
